...
$sourceFolder = Join-Path $basePath ".\OSDCloudCustomBuilder" | Resolve-Path -ErrorAction Stop
$diagFolder   = Join-Path $sourceFolder "Diagnostics"

Invoke-ScriptQualityCheck `
    -DirectoryPath $sourceFolder `
    -ExcelPath     (Join-Path $diagFolder "OSDCloudCustomBuilder_report_summary.xlsx") `
    -DbPath        (Join-Path $diagFolder "OSDCloudCustomBuilder_report_summary.db")

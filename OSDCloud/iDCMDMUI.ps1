...
############################################################################
function Find-CustomWimFiles {
    # Check for custom.wim in common locations
    $possibleLocations = @(
        "X:\OSDCloud\custom.wim",
        "X:\custom.wim",
        "X:\Sources\custom.wim"
    )
    
    foreach ($location in $possibleLocations) {
        if (Test-Path $location) {
            # If found, copy to the standard location
            if ($location -ne "X:\OSDCloud\custom.wim") {
                $destinationFolder = "X:\OSDCloud"
                if (-not (Test-Path $destinationFolder)) {
                    New-Item -Path $destinationFolder -ItemType Directory -Force | Out-Null
                }
                Copy-Item -Path $location -Destination "X:\OSDCloud\custom.wim" -Force
            }
            return $true
        }
    }
    
    # Check for custom.wim on connected USB drives
    $usbDrives = Get-Volume | Where-Object { $_.DriveType -eq 'Removable' -and $_.DriveLetter }
    foreach ($drive in $usbDrives) {
        $driveLetter = $drive.DriveLetter
        $wimPath = "${driveLetter}:\custom.wim"
        if (Test-Path $wimPath) {
            $destinationFolder = "X:\OSDCloud"
            if (-not (Test-Path $destinationFolder)) {
                New-Item -Path $destinationFolder -ItemType Directory -Force | Out-Null
            }
            Copy-Item -Path $wimPath -Destination "X:\OSDCloud\custom.wim" -Force
            return $true
        }
    }
...
# Check for custom WIM files
$customwim = Find-CustomWimFiles
...
$OSDCloudBTN.Add_Click({
    ...
    if ($customwim) {
        Write-Host "Custom WIM file found, launching OSDCloud GUI"
    } else {
        Write-Host "No custom WIM file found, launching standard OSDCloud GUI"
    }
    
    start-process powershell -argumentlist "$PSScriptRoot\iDcMDMOSDCloudGUI.ps1"
...
#OSD Cloud
#start-process powershell -argumentlist "-NoL -W Mi -C Start-OSDCloudGUI -Brand 'iDcMDM OSD powered by OSDCloud'"
#iDcMDM OSD Cloud
start-process powershell -argumentlist "$PSScriptRoot\iDcMDMOSDCloudGUI.ps1"
})

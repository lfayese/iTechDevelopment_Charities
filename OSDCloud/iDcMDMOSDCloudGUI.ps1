...
# Add this after the hardware compatibility check section
#================================================
#   Custom WIM Check
#================================================
$CustomWimPath = "X:\OSDCloud\custom.wim"
$CustomWimExists = Test-Path $CustomWimPath
$CustomWimInfo = $null

if ($CustomWimExists) {
    try {
        $CustomWimInfo = Get-WindowsImage -ImagePath $CustomWimPath -Index 1 -ErrorAction Stop
    } catch {
        Write-Host "Custom WIM file exists but could not be read: $_" -ForeColor Yellow
        $CustomWimExists = $false
    }
}

# Modify the UI section to add a new label and checkbox for custom WIM
# Add this after the existing labels

$LabelCustomWim = New-Object System.Windows.Forms.Label
$LabelCustomWim.Location = New-Object System.Drawing.Size(20,285)
$LabelCustomWim.Font = [System.Drawing.Font]::new("Microsoft Sans Serif", 12, [System.Drawing.FontStyle]::Bold)
$LabelCustomWim.ForeColor = "Black"
$LabelCustomWim.Text = "Custom Windows Image"
$LabelCustomWim.AutoSize = $True

$CheckBoxCustomWim = New-Object System.Windows.Forms.CheckBox
$CheckBoxCustomWim.Location = New-Object System.Drawing.Size(190,285)
$CheckBoxCustomWim.Size = New-Object System.Drawing.Size(540,25)
$CheckBoxCustomWim.Font = [System.Drawing.Font]::new("Microsoft Sans Serif", 12, [System.Drawing.FontStyle]::Bold)
$CheckBoxCustomWim.Text = if ($CustomWimExists) { "Use custom.wim: $($CustomWimInfo.ImageName)" } else { "No custom.wim found" }
$CheckBoxCustomWim.Checked = $CustomWimExists
$CheckBoxCustomWim.Enabled = $CustomWimExists

# Adjust the position of the Start button and warning label
$formMainWindowControlStartButton.Location = New-Object System.Drawing.Size(20,330)
$Label3.Location = New-Object System.Drawing.Size(20,380)

# Modify the StartButton click handler to include custom WIM logic
...
Set-UEFIVariable -Namespace "{616e2ea6-af89-7eb3-f2ef-4e47368a657b}" -VariableName FORCED_NETWORK_FLAG -ByteArray $bytes
    ...
    $UseCustomWim = $CheckBoxCustomWim.Checked
    ...
    $Global:StartOSDCloudGUI = [ordered]@{
        ...
        ImageFileFullName           = if ($UseCustomWim) { $CustomWimPath } else { $null }
        ImageFileItem               = $null
        ImageFileName               = if ($UseCustomWim) { "custom.wim" } else { $null }
        ...
        OSLicense                   = 'Volume'
        OSName                      = if ($UseCustomWim) { "Custom Windows Image" } else { $OSName }
        OSVersion                   = $null
        Restart                     = $true
...
$objForm.Controls.Add($LabelCustomWim)
$objForm.Controls.Add($CheckBoxCustomWim)

# Adjust the form size to accommodate the new controls
$objForm.Size = New-Object System.Drawing.Size(755,435)
...
#   UI
#================================================
write-host "Starting iDCMDM OSDCloudGUI..."
...
$objForm.Text = "iDCMDM OSD powered by OSDCloud"
